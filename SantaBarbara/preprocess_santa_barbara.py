...
def clean_trans(trans):
    ...
    for t in trans:
        ...
        # Update: we want to keep these words!
        ...
        # there are no words in the dictionary with dash, replace them with space
        t = t.replace("_", "-")
        t = t.replace("-", " ")
        if t:
            new_trans.append(t)
...
data_dir = r"../tests/sample_data/Santa_Barabara"
output_dir = r"../tests/sample_data/Santa_Barabara_ready_for_MFA"
...
for root, directories, files in os.walk(data_dir):
    for trn in sorted(files):
        ...
        intervals = {x: textgrid.IntervalTier(x, maxTime=duration) for x in speakers}
        for s, turns in transcriptions.items():
            ...
            for t in turns:
                if cur_interval is None:
                    ...
                    if not mark:
                        continue
                    cur_interval = textgrid.Interval(t[0], t[1], mark)
                else:
                    mark, breath_start = clean_trans(t[2])
                    if not mark:
                        continue
                    ...
                    if breath_start or t[0] - cur_interval.maxTime > 0.2:
                        ...
                        cur_interval = textgrid.Interval(begin, end, mark)
                    else:
